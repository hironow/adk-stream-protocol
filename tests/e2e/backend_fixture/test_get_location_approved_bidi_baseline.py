...
Tests backend behavior against get_location BIDI baseline fixture with approval flow.
...
async def test_get_location_approved_bidi_baseline(frontend_fixture_dir: Path):
    """Should generate correct rawEvents for get_location approval flow (BIDI)."""
    ...
    async with websockets.connect(
        ...
        # Construct approval message with tool result (frontend-delegated pattern)
        ...
        approval_msg = create_approval_message(confirmation_id, original_id, tool_result=tool_result)
        ...
        is_match, diff_msg = compare_raw_events(
            ...
            normalize=True,
            dynamic_content_tools=["get_location"],  # Tool output has dynamic content
            include_text_events=False,  # Ignore text-* events (thought process is non-deterministic)
        )
