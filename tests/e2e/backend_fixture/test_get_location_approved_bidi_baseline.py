...
- User: Approves (via frontend confirmation)
- Backend: location data → [DONE] (second turn)

Note: This fixture captures BOTH turns in a single expected output.
The fixture documents the complete approval flow from request to execution.
"""
...
async def test_get_location_approved_bidi_baseline(frontend_fixture_dir: Path):
    ...
    # And: Should have exactly 2 [DONE] markers (two-turn: confirmation + execution)
