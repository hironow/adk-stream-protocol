...
test.describe("Tool Approval SSE Mode (Core)", () => {
  ...
  test("should display tool name and arguments in approval UI", async ({
    page,
  }) => {
  ...
  test("should show approval state after clicking approve", async ({
    page,
  }) => {
    // Given: Message sent
    const chatInput = page.locator('input[placeholder="Type your message..."]');
    ...
    if (hasApproveButton) {
      ...
      const _buttonStillVisible = await approveButton
        .isVisible()
        .catch(() => false);
  ...
  test("should display tool result after approval and execution", async ({
    page,
  }) => {
    // Given: Message sent
    const chatInput = page.locator('input[placeholder="Type your message..."]');
  ...
  test("should handle approval of tools with complex arguments", async ({
    page,
  }) => {
    // Given: Message with complex requirements
    const chatInput = page.locator('input[placeholder="Type your message..."]');
    await chatInput.fill(
      "Analyze data from multiple sources and generate a report",
    );
    await chatInput.press("Enter");
    ...
    for (const button of approveButtons.slice(0, 3)) {
      // Limit to first 3
  ...
  test("should allow canceling/denying after initial approval request", async ({
    page,
  }) => {
    // Given: Message sent
    const chatInput = page.locator('input[placeholder="Type your message..."]');
  ...
  test("should maintain message order with tool approvals", async ({
    page,
  }) => {
  ...
  test.skip("should handle tool approval timeout", async ({ page }) => {
    ...
    const _hasTimeoutMessage =
      (await page.locator("text=/timeout/i, text=/expired/i").count()) > 0;
  });
});
