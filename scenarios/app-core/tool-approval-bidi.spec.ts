...
test.describe("Tool Approval BIDI Mode (Core)", () => {
  ...
  test("should handle multiple frontend tools in sequence", async ({
    page,
  }) => {
    // Given: BIDI mode
  ...
  test("should show tool results inline with conversation", async ({
    page,
  }) => {
  ...
  test.skip("should handle audio playback for tool results", async ({
    ...
    const _hasAudioControls =
      (await page.locator('[data-testid*="audio"], [class*="audio"]').count()) >
      0;
  });
});
