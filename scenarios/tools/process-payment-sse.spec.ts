 ...
 *
 * Purpose: Cover all critical scenarios with minimal test cases
...
test.describe("ADK Tool Confirmation - Minimal Test Suite", () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    await downloadFrontendChunkLogs(page, `process-payment-sse-${testName}`);
  });
  ...
  test("1. Normal Flow - Approve Once", async ({ page }) => {
    ...
    console.log("[Test 1] Requesting payment...");
  ...
  test("2. Denial Flow - Deny Once (CRITICAL: No infinite loop)", async ({
    ...
    console.log("[Test 2] Requesting payment...");
