...
| Date | Experiment | Status | Objective | Result |
|------|-----------|--------|-----------|--------|
| 2025-12-16 | [sendAutomaticallyWhen Transport Recreation Hypothesis](./2025-12-16_sendautomatically_transport_recreation_hypothesis.md) | ✅ **Complete** | Restore AI SDK v6 `sendAutomaticallyWhen` after transport fix | ✅ Hypothesis confirmed: transport recreation was root cause. `sendAutomaticallyWhen` works via `lib/core/send-automatically-when.ts`. Manual send was never in production (only obsolete tests). Removed `tool-invocation.test.tsx`. |
| 2025-12-15 | [Mode × Message Type Matrix Testing](./2025-12-15_mode_message_type_matrix_testing.md) | ✅ **Complete** | Test 5×3 matrix and mode transitions | ✅ ADK mode history sharing implemented. E2E tests: `chat-history-adk-modes.spec.ts` (5/5 passing). Scope limited to ADK SSE ↔ ADK BIDI transitions; Gemini Direct transitions deferred (different schema). |
