...
test.describe('Tool Execution UI (Core)', () => {
  ...
  test('should display tool invocation UI when tool is called', async ({ page }) => {
  ...
  test('should show approval buttons for tool requiring confirmation', async ({ page }) => {
  ...
  test('should show multiple tool invocations in sequence', async ({ page }) => {
    // Given: ADK SSE mode
    const adkSseButton = page.getByRole('button', { name: /ADK SSE/i });
    await adkSseButton.click();
  ...
  test('should preserve tool results when switching modes', async ({ page }) => {
  ...
  test.skip('should handle tool execution errors gracefully', async ({ page }) => {
    ...
    const hasError = await page.locator('text=/error/i, text=/failed/i').count() > 0;
