...
test.describe('Tool Approval Mixed Scenarios (Core)', () => {
  ...
  test('should switch from SSE approval mode to BIDI automatic mode', async ({ page }) => {
    ...
    const noApprovalButtons = await page.locator('button:has-text("Approve"), button:has-text("approve")').count() === 0;
  ...
  test('should switch from BIDI automatic mode to SSE approval mode', async ({ page }) => {
  ...
  test('should preserve message history across mode switches with tools', async ({ page }) => {
    const chatInput = page.locator('input[placeholder="Type your message..."]');
  ...
  test('should handle pending approval when switching modes', async ({ page }) => {
  ...
  test('should handle tool execution across Gemini, SSE, and BIDI modes', async ({ page }) => {
    const chatInput = page.locator('input[placeholder="Type your message..."]');
    ...
    // Approve if needed
    const sseApproveButton = await page.locator('button:has-text("Approve"), button:has-text("approve")').count();
  ...
  test('should handle approval then denial in SSE mode', async ({ page }) => {
    ...
    const firstApprove = await page.locator('button:has-text("Approve"), button:has-text("approve")').count();
    if (firstApprove > 0) {
      await page.locator('button:has-text("Approve"), button:has-text("approve")').first().click();
      await page.waitForTimeout(3000);
    }
    ...
    const denyButton = await page.locator('button:has-text("Deny"), button:has-text("deny")').count();
    if (denyButton > 0) {
      await page.locator('button:has-text("Deny"), button:has-text("deny")').first().click();
      await page.waitForTimeout(2000);
    }
  ...
  test('should display correct UI for each mode after switching', async ({ page }) => {
    const chatInput = page.locator('input[placeholder="Type your message..."]');
  ...
  test('should handle errors during mode switch with pending tools', async ({ page }) => {
  ...
  test('should maintain separate tool execution contexts per mode', async ({ page }) => {
    const chatInput = page.locator('input[placeholder="Type your message..."]');
