...
test.describe('Audio and Multimodal (Advanced)', () => {
  ...
  test('should handle audio completion notifications in BIDI mode', async ({ page }) => {
    // Given: BIDI mode
    const adkBidiButton = page.getByRole('button', { name: /ADK BIDI/i });
    await adkBidiButton.click();
  ...
  test('should handle BGM and voice simultaneously in BIDI mode', async ({ page }) => {
    // Given: BIDI mode
    const adkBidiButton = page.getByRole('button', { name: /ADK BIDI/i });
    await adkBidiButton.click();
  ...
  test('should display image preview before sending', async ({ page }) => {
    ...
    const fileButtons = await page.locator('label:has-text("Attach Image"), button:has-text("Attach")').all();
  ...
  test('should allow removing attached image before sending', async ({ page }) => {
