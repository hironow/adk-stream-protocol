...
test.describe('Error Handling UI (Advanced)', () => {
  ...
  test('should display error message when API fails', async ({ page }) => {
    ...
    await expect(page.locator('text=Test message for error handling')).toBeVisible();
  ...
  test('should handle tool execution errors gracefully in SSE mode', async ({ page }) => {
    // Given: SSE mode
  ...
  test('should handle tool execution errors gracefully in BIDI mode', async ({ page }) => {
    // Given: BIDI mode
  ...
  test('should handle WebSocket connection errors in BIDI mode', async ({ page }) => {
    // Given: BIDI mode
    const adkBidiButton = page.getByRole('button', { name: /ADK BIDI/i });
    await adkBidiButton.click();
  ...
  test('should recover from WebSocket disconnection in BIDI mode', async ({ page }) => {
    // Given: BIDI mode
    const adkBidiButton = page.getByRole('button', { name: /ADK BIDI/i });
    await adkBidiButton.click();
    ...
    await chatInput.press('Enter');
    await expect(page.locator('text=before potential disconnect')).toBeVisible();
    await page.waitForTimeout(3000);
  ...
  test('should handle concurrent errors from multiple tools', async ({ page }) => {
